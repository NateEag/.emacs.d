(custom-set-variables
 ...
 '(ac-trigger-commands-on-completing
   '(delete-backward-char backward-delete-char backward-delete-char-untabify
                          autopair-backspace paredit-backward-delete
                          paredit-backward-delete-word smart-dash-insert
                          ne-smart-dash-hacks-sh-mode-insert))
 '(ac-trigger-key "TAB")
 '(afp-always-fill-on-self-insert t)
 '(afp-fill-after-functions
   '(evil-delete backward-delete-char backward-delete-char-untabify kill-region
                 evil-change yank yank-pop evil-paste-after))
 '(afp-fill-comments-only-mode-list
   '(emacs-lisp-mode sh-mode python-mode js-mode php-mode xml-mode nxml-mode
                     diff-mode gitconfig-mode lua-mode yaml-mode ruby-mode))
 '(afp-fill-keys
   '(32 46 97 98 99 100 101 102 103 104 105 106 107 108 109 110 111 112 113 114
        115 116 117 118 119 120 121 122 65 66 67 68 69 70 71 72 73 74 75 76 77
 ...
 '(elfeed-feeds
   '("https://www.existentialcomics.com/rss.xml" "https://www.mtjoytwp.org/feed/"))
 ...
 '(notmuch-saved-searches
   '((:name "Learn Tech" :query "tag:learn-tech")
     (:name "Third-party software" :query "tag:third-party-software"
            :sort-order nil)
     ...
     (:name "Unread Emails In Watched Threads" :query
            "tag:watched and tag:unread" :key "w")
     ...
     (:name "Sent" :query "from:neagleson@nxtbookmedia.com" :sort-order
            newest-first :search-type nil)))
 ...
 '(package-selected-packages
   '(ac-emmet ac-helm ac-html ac-html-csswatcher ac-ispell ac-slime accent
              ...
              backup-each-save backup-walker bind-key browse-url-dwim
              bug-hunter cask-mode compact-docstrings csv cycle-quotes
              cygwin-mount dart-mode delight do-at-point dot-env dynamic-spaces
              el2markdown eldoc-overlay-mode elm-mode elpygen esup evil-escape
              evil-indent-textobject evil-leader flycheck-css-colorguard
              focus-autosave-mode forge frame-cmds free-keys git-blamed
              git-gutter-fringe+ git-walktree gitattributes-mode gitconfig-mode
              gitignore-mode gnuplot-mode helm-git-files hideshowvis iss-mode
              js-doc jscs kotlin-mode literate-coffee-mode lsp-dart magit
              magit-delta magit-stats minimap mo-git-blame origami osx-plist
              php-auto-yasnippets puppet-mode pyimpsort regex-tool
              reveal-in-osx-finder ripgrep sane-term scss-mode sexp-diff
              skewer-reload-stylesheets slack smart-tabs-mode sublimity swagg
              tagedit tea-time toggle-quotes undo-tree url-http-oauth
              use-package vcard xref xterm-color yasnippet-classic-snippets))
 ...
 '(safe-local-variable-values
   '((mangle-whitespace . t) (checkdoc-minor-mode . t)
     ...
     (flycheck-eslint-rulesdir concat nateeag/dir-locals-dir
                               "node_modules/camel_case")
     (eval set (make-local-variable 'nateeag/dir-locals-dir)
           (file-name-directory
            (let ((d (dir-locals-find-file "."))) (if (stringp d) d (car d)))))
     (flycheck-eslint-rulesdir concat nateeag/project-path
                               "node_modules/camel_case")
     (eval set (make-local-variable 'nateeag/project-path)
           (file-name-directory
            (let ((d (dir-locals-find-file "."))) (if (stringp d) d (car d)))))
     (scss-compile-at-save) (eval add-to-list 'after-save-hook 'hit-servlet)
 ...
 '(yas-expand-only-for-last-commands
   '(self-insert-command smart-dash-insert web-mode-smart-dash-insert ac-complete
                         ac-expand) nil nil "Only trigger yasnippet while first typing. Makes TAB indent even if inadvertantly over a yasnippet's key.")
