...
(defmacro elpaca-test-package-stub (name &rest args)
  "Create a package stub with NAME and ARGS."
  (declare (indent 1) (debug (symbolp body)))
  (let* ((elpaca-test--keywords '(:requires :body :version :path))
         (args (elpaca-test--args args))
         (pkgname (concat (symbol-name name) ".el"))
         (version (or (car (plist-get args :version)) "0.0.0"))
         (command-name (format "%s-test-command" name))
         (forms (or (if-let* ((declared (plist-get args :body))
                              ((consp declared)))
                        declared
                      `(";;;###autoload"
                        (defun ,(intern command-name) ()
                          "Generated by `elpaca-test-package-stub'."
                          (interactive)
                          (message "%s: COMMAND PASS" ,command-name))))))
         (deps (if-let* ((deps (plist-get args :requires))
                         ((consp deps))
                         ((consp (car-safe deps))))
                   deps
                 (when deps (list deps))))
         (dependencies (cl-loop for (dep version) in deps collect
                                (list dep (format "%S" (or version 0)))))
         (path (car-safe (plist-get args :path)))
         (text (with-temp-buffer
                 (cl-loop for section in (list
                                          (format ";;; %s  --- Stub package generated by elpaca-test -*- lexical-binding: t; -*-\n" name)
                                          (format-time-string ";; Created: %F %H:%M:%S")
                                          (format ";; Version: %s" version)
                                          (when deps (format ";; Package-Requires: %S" dependencies)))
                          when section do (insert section "\n"))
                 (insert "\n;;; Code:\n")
                 (unless (consp (plist-get args :body))
                   (cl-loop for (dep _) in deps do (insert (format "(require '%s)" dep) "\n")))
                 (insert "\n")
                 (cl-loop for form in forms do (insert (if (stringp form) form (pp-to-string form)) "\n"))
                 (insert (format "(provide '%s)\n" name))
                 (insert (format ";;; %s ends here" pkgname))
                 (buffer-substring-no-properties (point-min) (point-max)))))
    `(if-let* ((pkg ,text)
               (path ,path)
